...
#define DELAY_US(n)	delay_us(n)					/* Delay n microseconds */
#define	IF_INIT()					/* Initialize control port */
#define	IF_DLY60()	SysCtlDelay(2);			/* Delay >=60ns (can be blanked for most uC) */
#define	IF_DLY450()	SysCtlDelay(8);			/* Delay >=450ns@3V, >=250ns@5V */
#define E1_HIGH()		GPIOPinWrite(GPIO_PORTA_BASE,GPIO_PIN_3,GPIO_PIN_3)	/* Set E(E1) high */
#define E1_LOW()		GPIOPinWrite(GPIO_PORTA_BASE,GPIO_PIN_3,~GPIO_PIN_3)	/* Set E(E1) low */
#define E2_HIGH()		GPIOPinWrite(GPIO_PORTB_BASE,GPIO_PIN_4,GPIO_PIN_4)		/* Set E2 high (dual controller only) */
#define E2_LOW()		GPIOPinWrite(GPIO_PORTB_BASE,GPIO_PIN_4,~GPIO_PIN_4)	/* Set E2 low (dual controller only) */
#define	RS_HIGH()		GPIOPinWrite(GPIO_PORTA_BASE,GPIO_PIN_2,GPIO_PIN_2)	/* Set RS high */
#define	RS_LOW()		GPIOPinWrite(GPIO_PORTA_BASE,GPIO_PIN_2,~GPIO_PIN_2)	/* Set RS low */
#define	OUT_DATA(d) GPIOPinWrite(GPIO_PORTA_BASE,0XF0,d)	/* Output a byte d on the bus (higher 4 bits of d in 4-bit mode) */
