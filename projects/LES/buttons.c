...
//
//*****************************************************************************
uint8_t buttons_poll(uint8_t *pui8Delta, uint8_t *pui8RawState)
{
    uint32_t ui32Delta;
    uint32_t ui32Data;
    ...
    // raw value.
    //
	ui32Data = ROM_GPIOPinRead(GPIO_PORTC_BASE, GPIO_PIN_4 | GPIO_PIN_5);
	ui32Data>>=2;
  ui32Data |= ROM_GPIOPinRead(GPIO_PORTF_BASE, GPIO_PIN_0 | GPIO_PIN_4);
...
buttons_init(void)
{
    ...
    //
    ROM_GPIODirModeSet(BUTTONS_GPIO_BASE,  GPIO_PIN_0|GPIO_PIN_4, GPIO_DIR_MODE_IN);
    MAP_GPIOPadConfigSet(BUTTONS_GPIO_BASE,  GPIO_PIN_0|GPIO_PIN_4,GPIO_STRENGTH_2MA, GPIO_PIN_TYPE_STD_WPU);
		GPIOPinTypeGPIOInput(GPIO_PORTC_BASE,GPIO_PIN_4|GPIO_PIN_5);
		MAP_GPIOPadConfigSet(GPIO_PORTC_BASE,GPIO_PIN_4|GPIO_PIN_5,GPIO_STRENGTH_2MA, GPIO_PIN_TYPE_STD_WPU);
    //
    // Initialize the debounced button state with the current state read from
    // the GPIO bank.
    //
    g_ui8ButtonStates = ROM_GPIOPinRead(BUTTONS_GPIO_BASE, GPIO_PIN_0|GPIO_PIN_4);
	  g_ui8ButtonStates|=ROM_GPIOPinRead(GPIO_PORTC_BASE,GPIO_PIN_4|GPIO_PIN_5)<<16;
}
