...
enum
{
    M0=0,MM,AM,US,FS,CS,AT,FT,FR,FP,BP,EF,RI,FE,CE,
...
const state_t FSM[SIZE]={
    {&main_menu          ,{M0,MM,AM,US,FS,EF,LC,DT,SS,M0}},
    {&maual_mode         ,{EF,RM,RM,RM,MM,MM,MM,MM,MM,MM}},
    {&auto_mode          ,{EF,RM,RM,RM,AM,AM,AM,AM,AM,AM}},
    {&user_settings      ,{M0,RI,FE,CE,TS,MS,ME,DG,US,US}},
    {&factory_settings   ,{M0,CS,LL,SF,SP,FS,FS,FS,FS,FS}},
...
//        switch(keyVal){
//            case 1:    dh->pfnLcdCursor(r,c+i,CURSOR_UNDER);temp[i]=value[j];if(j>=(strlen(value)-1))j=0;j++;dh->pfnLcdPrintChr(r,c+i,temp[i]);break;

//            case 2:    dh->pfnLcdCursor(r,c+i,CURSOR_UNDER);    temp[i]=value[j];if(j<=0)j=(strlen(value)-1);j--;dh->pfnLcdPrintChr(r,c+i,temp[i]);break;

//            case 3:    dh->pfnLcdCursor(r,c+i,CURSOR_BLOCK);    i-=1;if(i<0){done=0;}break;

//            case 4:    dh->pfnLcdCursor(r,c+i,CURSOR_BLOCK);    i+=1;if(i>=(strlen(temp))){done=0;i=0;dh->pfnLcdPrintStr(r,c+i,temp); sscanf(temp,"%f\n",&retNum);}break;

//            case 9: dh->pfnLcdCursor(r,c+i,CURSOR_BLOCK);break;
...
static uint32_t main_menu(void){
    ...
    dh->pfnLcdPrintStr(2,0, " USER SETTINGS      ");
    dh->pfnLcdPrintStr(3,0, " FACTORY SETTING    ");
    dh->pfnLcdPrintStr(0,20," FILTER ID          ");
    dh->pfnLcdPrintStr(1,20," LEAK CHECK         ");
    dh->pfnLcdPrintStr(2,20," DATA TRANSFER      ");
    dh->pfnLcdPrintStr(3,20," SAMPLING SUMMARY   ");
    
    
    ret_val=menu_scanner(7,profiler);
    if(ret_val==4){
        if(Password()==0){
            ret_val=0;
        }
...
static uint32_t user_settings(void){
    ...
    dh->pfnLcdPrintStr(2,0, " COMPENSATION        ");
    dh->pfnLcdPrintStr(3,0, " CLOCK SETTINGS      ");
    dh->pfnLcdPrintStr(0,20," MEMORY STATUS       ");
    dh->pfnLcdPrintStr(1,20," MEMORY ERASE        ");
    dh->pfnLcdPrintStr(2,20," DIAGNOSTICS         ");
    dh->pfnLcdPrintStr(3,20,"                     ");
...
static uint32_t edit_rec_interval(void){
    ...
    dh->pfnLcdClear();
    dh->pfnLcdPrintStr(0,10, "SAMPLING INTERVAL");
    edit_integer_value_min_max(&user_data.rec_int,"%02d Min",1,16,1,60,1);
    dh->pfnLcdPrintStr(2,16, "SAVING..");
    set_struct(&user_data,sizeof(user_settings_t),USERSETTINGS);
    dh->pfnDelay(80000000/6);
    dh->pfnLcdPrintStr(2,16, "SAVED.. ");
    dh->pfnDelay(80000000/6);
...
static uint32_t edit_flow_rate(void){
    ...
    dh->pfnLcdClear();
    dh->pfnLcdPrintStr(0,14, " FLOW RATE ");
    dh->pfnLcdPrintStr(1,20, " Lpm ");
    edit_float_value(&user_data.set_fr,"%4.1f",1,16);
    dh->pfnLcdPrintStr(2,16, "SAVING..");
    set_struct(&user_data,sizeof(user_settings_t),USERSETTINGS);
    dh->pfnDelay(80000000/6);
    dh->pfnLcdPrintStr(2,16, "SAVED.. ");
...
static uint32_t comp_on_off(void){
    ...
    dh->pfnLcdClear();
    dh->pfnLcdPrintStr(1,10, "COMPENSATION");

    if(user_data.comp == 1)
    {
        dh->pfnLcdPrintStr(1,23, ":ON   ");
    }
    else
    {
        dh->pfnLcdPrintStr(1,23, ":OFF ");
    }
...
static uint32_t time_settings(void){
    ...
    */
    ...
    const uint8_t col[6]={18,21,24,18,21,24};    // col coordinate
    ...
    */
    ...
    dh->pfnLcdClear();
    dh->pfnLcdPrintStr(0,10,"Enter New Date Time");
    sprintf(str,"Date:%02d/%02d/%02d",dt[0],dt[1],dt[2]);

    dh->pfnLcdPrintStr(2,13,str);

    sprintf(str,"Time:%02d:%02d:%02d",dt[3],dt[4],dt[5]);
    dh->pfnLcdPrintStr(3,13,str);
        ...
        switch(key_value){
            ...
            case 4:{
                ...
                if(i>=6){
                    ...
                    dh->pfnLcdClear();
                    dh->pfnLcdPrintStr(1,14,"UPDATING...");
                    /*
                    update the RTC
                    */
                    cal.dd=dt[0];
...
static uint32_t diagnostics(void){
    ...
    dh->pfnLcdClear();
    dh->pfnLcdPrintStr(2,14,"PRESS ENTER -> PUMP ON/OFF");
    dh->pfnLcdPrintStr(3,14,"PRESS UP/DN -> FLOW ADJUST");
    dh->pfnIntEnable01Hz();
...
static int32_t Password(void){
	...
	dh->pfnLcdPrintStr(0,10,"   Enter Password   ");
	strcpy(temp,"0000");
	edit_string_1(temp,1,18);
	...
	{
		dh->pfnLcdPrintStr(2,10,"   Access Granted   ");
		ret=1;
	}
	
	else{
		dh->pfnLcdPrintStr(2,10,"   Access Denied    ");
		ret=0;
	}
...
static uint32_t factory_settings(void){
    ...
    dh->pfnLcdPrintStr(0,20,"                    ");
    dh->pfnLcdPrintStr(1,20,"                    ");
    dh->pfnLcdPrintStr(2,20,"                    ");
    dh->pfnLcdPrintStr(3,20,"                    ");
...
static uint32_t edit_filter_id(void){
    ...
    dh->pfnLcdPrintStr(0,12,"EDIT FILTER ID:");
...
static uint32_t auto_setting_state(void){
    const uint8_t row[12]={2,2,2,3,3,3,2,2,2,3,3,3};                // row coordinate
	const uint8_t col[12]={11,14,17,11,14,17,31,34,37,31,34,37};	// col coordinate
	//dd/mm/yy hh:mm:ss
...
static uint32_t auto_exit_state(void){
	...
	dh->pfnLcdClear();
	dh->pfnLcdPrintStr(0,10,"  TO EXIT AUTO MODE     ");
	dh->pfnLcdPrintStr(1, 0,"                        ");
	dh->pfnLcdPrintStr(2,10,"     PRESS ENTER         ");
	dh->pfnLcdPrintStr(3,10,"        ..              ");
...
static uint32_t memory_erase(void){
    ...
    dh->pfnLcdClear();
    dh->pfnLcdPrintStr(0,0,"   !!!!!!!!!!!!ATTENTION!!!!!!!!!!!!!!!");
    dh->pfnLcdPrintStr(1,0,"   ALL DATA AND SUMMARY WILL BE ERASED");
    dh->pfnLcdPrintStr(2,0,"         PRESS ENTER TO DELETE        ");
    dh->pfnLcdPrintStr(3,0,"         PRESS MENU  TO SKIP          ");
...
static uint32_t sampling_summary(void){
  ...
  char str[32];
    ...
    dh->pfnLcdPrintStr(0,13,str);
	
	sprintf(str,"FR:%0.1f LPM\n",summary_data.afr);
    dh->pfnLcdPrintStr(0,26,str);
    
	sprintf(str,"FP:%0.0f mmHg\n",summary_data.afp);
	dh->pfnLcdPrintStr(1,0,str);
	
	sprintf(str,"CV:%0.2f %%\n",summary_data.cv);
	dh->pfnLcdPrintStr(1,13,str);
	
    get_struct(&user_data,sizeof(user_settings_t),USERSETTINGS);
	sprintf(str,"FID:%8s\n",user_data.f_id);
    dh->pfnLcdPrintStr(1,26,str);
...
static uint32_t leak_check_limit(void){
    ...
    dh->pfnLcdClear();
    dh->pfnLcdPrintStr(1,10,"LEAK LIMIT:    mmhg");
    edit_integer_value_min_max(&calib_data.leak_chk_limit,"%03d",1,21,1,100,1);
    dh->pfnLcdPrintStr(3,16, "SAVING..");
    set_struct(&calib_data,sizeof(calibration_t),CALIBRATION);
    dh->pfnDelay(50000000/6);
    dh->pfnLcdPrintStr(3,16, "SAVED.. ");
    dh->pfnDelay(50000000/6);
...
static uint32_t shdn_flow_rate(void){
    ...
    dh->pfnLcdClear();
    dh->pfnLcdPrintStr(1,0, "    SHUTDOWN FLOW:      Lpm");
   edit_float_value(&calib_data.shdn_fr,"%05.1f",1,18);
    dh->pfnLcdPrintStr(2,16, "SAVING..");
    set_struct(&calib_data,sizeof(calibration_t),CALIBRATION);
    dh->pfnDelay(50000000/6);
    dh->pfnLcdPrintStr(2,16, "SAVED.. ");
...
static uint32_t shdn_pressure(void){
    ...
    dh->pfnLcdClear();
    dh->pfnLcdPrintStr(1,0,"    SHUTDOWN PRES:         mmhg");
    edit_float_value(&calib_data.shdn_fp,"%05.1f",1,18);
    dh->pfnLcdPrintStr(2,16, "SAVING..");
    set_struct(&calib_data,sizeof(calibration_t),CALIBRATION);
    dh->pfnDelay(50000000/6);
    dh->pfnLcdPrintStr(2,16, "SAVED.. ");
...
uint32_t run_module(void){
    ...
    if(auto_mode_data.auto_mode==1)
    {
        ...
        dh->pfnGetClock(&cal);
        dh->pfnLcdPrintStr(0,39,"A");
        dh->pfnLcdPrintStr(1,39,"U");
        dh->pfnLcdPrintStr(2,39,"T");
        dh->pfnLcdPrintStr(3,39,"O");
    ...
    {
        dh->pfnLcdPrintStr(0,39,"M");
        dh->pfnLcdPrintStr(1,39,"A");
        dh->pfnLcdPrintStr(2,39,"N");
        dh->pfnLcdPrintStr(3,39," ");
    }
    rtc_ticks=0;
    sample_avg.at=0.0f;
    sample_avg.bp=0.0f;
    ...
    do{
        ...
        if(rtc_event){
            ...
            dh->pfnGetClock(&cal);
            sprintf(str,"CD:%02d/%02d/%02d",cal.dd,cal.mm,cal.yy);
            dh->pfnLcdPrintStr(2,27,str);
            sprintf(str,"CT:%02d:%02d:%02d",cal.h,cal.m,cal.s);
            dh->pfnLcdPrintStr(3,27,str);
            ...
            summary_data.et++;
            sprintf(str,"ET:%02d:%02d  Hr",summary_data.et/3600,((summary_data.et%3600)/60));
            dh->pfnLcdPrintStr(2,0,str);

            sprintf(str,"TV:%06.3f m3",summary_data.tv);
            dh->pfnLcdPrintStr(3,0,str);

            sprintf(str,"AT:%04.1f  'C",avg.at);
            dh->pfnLcdPrintStr(0,14,str);

            sprintf(str,"FT:%04.1f  'C",avg.ft);
            dh->pfnLcdPrintStr(1,14,str);

            sprintf(str,"FP:%03d mmHg",(int)avg.fp);
            dh->pfnLcdPrintStr(2,14,str);

            sprintf(str,"BP:%04d mBar",(int)avg.bp);
            dh->pfnLcdPrintStr(3,14,str);
...
void dusthawk_start(void){
    ...
    dh->pfnLcdPrintStr(0,0," Envirotech Instruments Private Limited ");
    dh->pfnLcdPrintStr(1,0,"  A-271 Okhla Industrial Area Phase -1  ");
    dh->pfnLcdPrintStr(2,0,"         **** 550 MFC ****              ");
    dh->pfnLcdPrintStr(3,0,"                                        ");
    ...
    if(dh->pfnCheckDisk(0) !=0 ){
        ...
        dh->pfnLcdPrintStr(1,0,"      DATA WILL NOT BE LOGGED        ");
        dh->pfnLcdPrintStr(2,0,"     <PRESS ANY KEY TO PROCEED>       ");
        WAIT_FOR_ANY_KEY();
        WAIT_FOR_ANY_KEY();
    }
